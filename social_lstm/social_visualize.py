...
def plot_trajectories(true_trajs, pred_trajs, obs_length, name):
            ...
            if true_pos[j][0] == 0:
                ...
                if j in traj_data:
                    ...
                    if fran:
                        ...
                        if i >= obs_length:
                            pred_data[j].append(list(pred_trajs[i][j][1:3]))
                        else:
    ...
    for i in range(traj_length):
        ...
        if i>= obs_length:
            pred_x[i-8] = (1. + pred_data[1][i-8][0]) * width / 2.
            pred_y[i-8] = (1. + pred_data[1][i-8][1]) * height / 2.
        if i < obs_length:
    ...
    ax1.plot(obs_x, obs_y, color='g',linestyle='solid', marker='o', label=label1)
    #Predicted trajectories
    ax1.plot(pred_x, pred_y, color='b', linestyle='solid', marker='o', label=label2)
    # Ploting real/observed trajectory from t till t+8
    ax1.plot(true_x, true_y, color='b',linestyle='solid', marker='_', label=label3)
